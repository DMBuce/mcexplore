...
If mcexplore is in your PATH, you run it from a server directory with a valid
an existing minecraft world in your server directory, your
server jar is called minecraft_server.jar, and you want to pregenerate a
63x63 chunk square of land (1008x1008 blocks) that's centered on spawn, you
can run
...
To do the same in a more verbose way,
 ...
 /path/to/mcexplore -p /path/to/server -c "java -jar minecraft_server.jar nogui" -x 0 -z 0 63 63
